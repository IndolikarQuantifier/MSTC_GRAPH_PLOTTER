...
int CreateBarGraph(HWND hWnd,LPSIMPLEBARGRAPH* lpSimpleBarGraph,unit* Heights,LPCOLORREF lpColor,unit scale,size_t iSize,const LPSTR xTitle,const LPSTR yTitle,const CHAR** lables)
{
    ...
    *lpSimpleBarGraph = (LPSIMPLEBARGRAPH) calloc(1,sizeof(SIMPLEBARGRAPH));
...
HWND Init(HINSTANCE hInstance,HWND hPrentWnd)
{
    ...
    wndGraphEx.lpfnWndProc = WndSimpleBarGraphProc;
...
{
    PRECT pRect = (PRECT) calloc(iSize,sizeof(RECT));
...
LRESULT CALLBACK WndSimpleBarGraphProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
    ...
    switch(uMsg)
    {
        ...
        tmpCoOrdinateAxis.Origin = (LPPOINT) calloc(1,sizeof(POINT));
        tmpCoOrdinateAxis.XAxisEnd = (LPPOINT) calloc(1,sizeof(POINT));
        tmpCoOrdinateAxis.YAxisEnd = (LPPOINT) calloc(1,sizeof(POINT));

        tmpScaleLine.StartPoint = (LPPOINT) calloc(1,sizeof(POINT));
        tmpScaleLine.EndPoint = (LPPOINT) calloc(1,sizeof(POINT));
